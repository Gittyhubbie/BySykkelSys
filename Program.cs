...
namespace BySykkelSys
{
    class Program
    {
        static RootObjectStationInfo jObjectStationInfo = new RootObjectStationInfo(); // these two objects will be used to contain our Jsons.
        static RootObjectStationStatus jObjectStationStatus = new RootObjectStationStatus();
        ...
        static void Main()
        {
            ...
            { jObjectStationInfo=JsonConvert.DeserializeObject<RootObjectStationInfo>(info); }   //Our aquired json-text is now converted to "StationInfo" objects

            catch(Exception ex) { Console.WriteLine(ex.Message); Environment.Exit(0); };


            string status=GetJson("https://gbfs.urbansharing.com/oslobysykkel.no/station_status.json");  // gets the content from given json file.

            try

            { jObjectStationStatus=JsonConvert.DeserializeObject<RootObjectStationStatus>(status); }    //Our aquired json-text is now converted to "StationStatus" objects
            ...
            var query = from sInfo in jObjectStationInfo.data.stations
                            join sStatus in jObjectStationStatus.data.stations
                                 on sInfo.station_id equals sStatus.station_id
                            orderby sStatus.num_bikes_available ascending
                            select new
                            {
                                sInfo.station_id,
                                sInfo.name,
            ...
            WriteLine("Table showing available rentabikes in the city of Oslo"); Console.WriteLine();
        ...
        public static string GetJson(string url)
        {
            using(var httpClient = new HttpClient())
            {
                ...
                catch (Exception ex) { WriteLine($"Check your connection. {url} {ex.Message}"); Environment.Exit(0); return null; }
